...
def _resolve_config_root(data_root: str | Path | None = None) -> Path:
    ...
    package_path = get_package_path("bt_api_base")
    if package_path is None:
        raise RuntimeError("Package path for 'bt_api_base' is not available")
...
def read_account_config() -> dict[str, Any]:
    ...
    if os.environ.get("CTP_ENV"):
        try:
            from bt_api_base.ctp_env_selector import apply_ctp_env
